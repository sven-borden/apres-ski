...
// Initial Apres-Ski schema. Mirrors the functional spec (PROJECT.md §2).
...
// PocketBase is internal-only (no public port); the Next.js server proxies all
// DB calls, the browser never hits PB directly (PROJECT.md §10).
//
// Conventions:
//  - Date-only values (startDate, endDate, meal/attendance date) stay `text`
//    in YYYY-MM-DD form to avoid timezone drift.
//  - Audit timestamps -> `autodate` (onCreate / onUpdate).
...
//    stay denormalized text/json — loose model, no relations.
...
migrate(
  (app) => {
    ...
    // ---- trips (single record, id "current") ---------------------------
    app.save(
      new Collection({
        type: "base",
        name: "trips",
    ...
    // ---- basecamp (single record, id "current") ------------------------
    app.save(
      new Collection({
        type: "base",
        name: "basecamp",
    ...
    // ---- meals (id = YYYY-MM-DD or "general") --------------------------
    app.save(
      new Collection({
        type: "base",
        name: "meals",
        ...open,
        fields: [
          { type: "text", name: "date", required: true }, // YYYY-MM-DD or "general"
          { type: "text", name: "tripId" },
          { type: "json", name: "responsibleIds" }, // string[]
          { type: "text", name: "description" },
          { type: "json", name: "shoppingList" }, // ShoppingItem[]
          { type: "bool", name: "excludeFromShopping" },
          { type: "text", name: "updatedBy" },
          { type: "autodate", name: "updatedAt", onCreate: true, onUpdate: true },
        ],
    ...
    // ---- weather (single record, id "la-tzoumaz") ----------------------
    app.save(
      new Collection({
        type: "base",
        name: "weather",
